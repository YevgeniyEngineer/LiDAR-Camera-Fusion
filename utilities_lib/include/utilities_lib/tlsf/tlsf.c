...
{
    return to_block((char *)ptr - offsetof(tlsf_block_t, header) - BLOCK_OVERHEAD);
}
...
static bool arena_grow(tlsf_t *t, size_t size)
{
    ...
    tlsf_block_t *block =
        to_block(t->size ? (char *)addr + t->size - 2 * BLOCK_OVERHEAD : (char *)addr - BLOCK_OVERHEAD);
    if (!t->size)
        block->header = 0;
...
void *tlsf_resize(tlsf_t *t, size_t req_size)
{
    ...
    // Calculate MAX_PAGES based on TLSF_MAX_SIZE and PAGE
    const size_t MAX_PAGES = 20 * TLSF_MAX_SIZE / PAGE;
    ...
    {
        start_addr =
            mmap(0, MAX_PAGES * PAGE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE | MAP_NORESERVE, -1, 0);
    }
